...
class Admin extends Admin_Controller
{
	...
	function index()
	{
		...
		$total_rows = $this->blog_entries_m->count_by(array('show_future'=>TRUE, 'status' => 'all'));
		$this->data->pagination = create_pagination('admin/blog/index', $total_rows);
		
		// Using this data, get the relevant results
		$this->data->news = $this->blog_entries_m->limit($this->data->pagination['limit'])->get_many_by(array(
			'show_future'=>TRUE,
			'status' => 'all'
		));
	...
	function create()
	{
		...
		{
			$id = $this->blog_entries_m->insert(array(
	...
	function edit($id = 0)
	{
		...
		$this->validation->set_rules($this->rules);
		$this->validation->set_fields();		
		$article = $this->blog_entries_m->get($id);
		
		if ($this->validation->run())
		{
			
			$result = $this->blog_entries_m->update($id, array(
	            'title'				=> $this->input->post('title'),
	            'slug'				=> $this->input->post('slug'),
	            'category_id'		=> $this->input->post('category_id'),
	            'intro'				=> $this->input->post('intro'),
	...
	{		
		$this->data->article = $this->blog_entries_m->get($id);
	...
	function publish($id = 0)
	{
		...
		if(!empty($ids))
		{
			...
			foreach ($ids as $id)
			{
				...
				if($article = $this->blog_entries_m->get($id) )
				{
					$this->blog_entries_m->publish($id);
					
					// Wipe cache for this model, the content has changed
					$this->cache->delete('blog_entries_m');				
	...
	{
		if(!$this->blog_entries_m->check_slug($slug))
		{
