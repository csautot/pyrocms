...
class Blog_entries_m extends MY_Model
{
    function get_all()
    {
    	$this->db->select('blog_entries.*, categories.title AS category_title, categories.slug AS category_slug');
       	$this->db->join('categories', 'blog_entries.category_id = categories.id', 'left');
    ...
    function get_archive_months()
    {
    	...
    	$this->db->distinct();
		$this->db->select('(SELECT count(id) FROM blog_entries t2 
		...
		$query = $this->db->get('blog_entries t1');
